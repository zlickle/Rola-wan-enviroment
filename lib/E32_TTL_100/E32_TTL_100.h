...
class E32_TTL_100 {
 public:
  E32_TTL_100();
  void E32_TTL_100_init(SoftwareSerial *serial,long baud);
  void blinkLED();
  ...
  bool ReadAUX();
  String ReceiveMsg(uint8_t *pdatabuf, uint8_t *data_len);
  void Reset_module();
  RET_STATUS SendMsg(uint8_t HIGH_ADDR, uint8_t LOW_ADDR, String data_send);
  RET_STATUS SettingModule(struct CFGstruct *pCFG,uint8_t HIGH_ADDR,uint8_t LOW_ADDR);
